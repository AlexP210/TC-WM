...
class PlanEvaluator:  # evaluator for planning
    ...
    def eval_actions(
        ...
        e_obses, e_states, e_rewards = self.env.rollout(self.seed, self.state_0, exec_actions)
        ...
        # plot trajs
        if self.wm.decoder is not None and self.n_plot_samples > 0:
            try:
    ...
    def _compute_rollout_metrics(self, e_state, e_obs, i_z_obs, e_rewards=None):
        """
        Args
            e_state: final state from env rollout
            e_obs: final obs from env rollout
            i_z_obs: final latent from WM rollout
            e_rewards: (n_evals, T) per-step rewards, or None
        Return
            logs
            successes
        """
        ...
        # Add cumulative reward metrics if available
        if e_rewards is not None:
            cumulative_rewards = e_rewards.sum(axis=1)  # (n_evals,)
            logs["mean_cumulative_reward"] = float(np.mean(cumulative_rewards))
            logs["max_cumulative_reward"] = float(np.max(cumulative_rewards))
            logs["min_cumulative_reward"] = float(np.min(cumulative_rewards))
            print(f"Cumulative reward: mean={logs['mean_cumulative_reward']:.3f}, "
                  f"max={logs['max_cumulative_reward']:.3f}, min={logs['min_cumulative_reward']:.3f}")
        ...
        if "proprio" in e_obs and "proprio" in self.obs_g:
        ...
        # Add dummy proprio if missing (robomimic env only returns visual)
        if "proprio" not in e_obs and "proprio" in self.obs_g:
            import torch as _torch
            proprio_shape = self.obs_g["proprio"].shape
            e_obs["proprio"] = np.zeros_like(self.obs_g["proprio"])
    ...
    def _plot_rollout_compare(
        ...
        correction = 0.15  # brightness offset to distinguish env (darker) from imagined
        ...
        if save_video:
            for idx in range(e_visuals.shape[0]):
                ...
                for i in range(e_visuals.shape[1]):
                    ...
                    goal = goal_visual[idx, 0].cpu()
                    # Layout: top=[env|goal], bottom=[imagined|goal]
                    # env row slightly darker to distinguish
                    top = torch.cat([e_obs - correction, goal - correction], dim=2)
                    bot = torch.cat([i_obs, goal], dim=2)
                    frame = torch.cat([top, bot], dim=1)
        ...
        # add a goal column; env row slightly darker to distinguish
